...
`dhl_login_code` enthält optional die `dhllogin://...` Weiterleitungs-URL aus dem DHL-Browser-Login. Nach dem ersten erfolgreichen Login speichert die App das Refresh-Token im App-Datenordner und liest die Paketliste des DHL-Kontos automatisch.
...
1. Öffne die DHL-Login-URL im Browser.
2. Melde dich mit deinem DHL-Konto an.
3. Kopiere aus der fehlgeschlagenen Weiterleitung die komplette `dhllogin://...` URL.
4. Füge diese URL in `dhl_login_code` ein und starte die App neu.
