...
`hose` offers exactly three values: `40 m`, `60 m` and `80 m`. `short_vehicle` is shown in Home Assistant as `Tankwagen` and offers `mit Anhänger möglich` or `ohne Anhänger`.
