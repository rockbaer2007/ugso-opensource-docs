...
- Heizöl to MQTT
- Parcel to MQTT
- [MQTT-Client](https://github.com/rockbaer2007/ugso-ha-mqtt-addons/blob/master/mqtt_client/DOCS.en.md): export selected HA states to ioBroker or other external brokers, with optional write commands. The existing HA broker stays in place. Version 0.1.7 shows a compact device search on the left; clicking a device opens a popup with control, sensors, configuration and diagnostics. It maps `press` to `button` and `state_boolean` to `switch`, and keeps bidirectional commands for controllable domains, value inputs and buttons.
